...
async function uploadCodemagicConfig(config: BuildRequest): Promise<boolean> {
  ...
  // Use raw strings and proper escaping for shell variables
        ...
        script: |
          ...
          async function processIcons() {
            ...
            if (!fs.existsSync(iconPath)) {
              console.log('No icon.png found, skipping');
              return;
            }
            ...
            const paddedSize = Math.floor(1024 * 0.7);
          ...
          processIcons().catch(e => console.error('Icon error:', e.message));
          ICONSCRIPT
    ...
    scripts:
      ...
      - name: Build iOS Archive
        script: |
          ...
          # Use for loop with -d test to detect directories (not files inside bundles)
          WORKSPACE=""
          PROJECT=""
          ...
          if [ -n "\$WORKSPACE" ]; then
            ...
            echo "Building: workspace=\$WORKSPACE scheme=\$SCHEME_NAME"
            ...
            echo "Building: project=\$PROJECT scheme=\$SCHEME_NAME"
